...
//        var sOut = ""
//        try {
//            val o_t_pubk = android.util.Base64.decode(publicKey, 0)
//            val rsaKey = KeyFactory.getInstance("RSA").generatePublic(X509EncodedKeySpec(o_t_pubk)) as RSAPublicKey
//            val modulus = BigInteger(rsaKey.getModulus().toString())
//            val expoment = BigInteger(rsaKey.getPublicExponent().toString())
//            val pubKeySpec = RSAPublicKeySpec(modulus, expoment)
//            val rsaPublicKey = KeyFactory.getInstance("RSA").generatePublic(pubKeySpec) as RSAPublicKey
//            val rsa_cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding")
//            rsa_cipher.init(Cipher.ENCRYPT_MODE, rsaPublicKey)
//            val enc_data = rsa_cipher.doFinal(txt.toByteArray())
